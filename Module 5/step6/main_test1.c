...
static int mode;// default mode


// forward anything recieved to uart 1
static void Uart0_Handler( void *CallBackRef, u32 Event, unsigned int EventData) {

	XUartPs *uart_ptr_0 = (XUartPs *) CallBackRef;
	u8 recieve_buffer;

	if (Event == XUARTPS_EVENT_RECV_DATA) {
		XUartPs_Recv(uart_ptr_0, &recieve_buffer, 1); // recieve normally
...
static void Uart1_Handler( void *CallBackRef, u32 Event, unsigned int EventData) {
	...
	XUartPs *uart_ptr_1= (XUartPs *) CallBackRef;
	u8 recieve_buffer_ptr;

	if (Event == XUARTPS_EVENT_RECV_DATA) {
		XUartPs_Recv(uart_ptr_1 , &recieve_buffer_ptr, 1);
					if(recieve_buffer_ptr == (u8)'\r') {
						...
						XUartPs_Send(uart_ptr_1 , &recieve_buffer_ptr, 1);
					} else{
						//XUartPs_Send(uart_ptr, &recieve_buffer_ptr, 1); // send to screen
						XUartPs_Send(&uart_ps_0, &recieve_buffer_ptr, 1); //send uart 0
					}
...
//			}
//		} else if (mode == PING) {
//			recieve_buffer_ptr = (u8)'p';
//			XUartPs_Send(uart_ptr_1, &recieve_buffer_ptr, 1);
//			return;
//		} else if (mode == UPDATE) {
//			recieve_buffer_ptr = (u8)'u';
//			XUartPs_Send(uart_ptr_1, &recieve_buffer_ptr, 1);
//			return;
//		}
	}
